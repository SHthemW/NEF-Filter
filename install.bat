...
if exist "%PROJECT_FILE%" (
  ...
  dotnet publish "%PROJECT_FILE%" -c Release -o "%PUBLISH_DIR%" || goto :fail
  ...
  if exist "%SCRIPT_DIR%NEF-Filter.exe" (
    ...
    echo ERROR: Could not find NEF-Filter.csproj or NEF-Filter.exe.
    goto :fail
  )
)

if not exist "%INSTALL_DIR%" mkdir "%INSTALL_DIR%" >nul 2>nul || goto :fail
